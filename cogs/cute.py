...
class Cute:
    ...
    async def quilt(self, ctx, *mems : discord.Member):
        ...
        avys = []
        for mem in mems:
            async with ctx.session.get(mems.avatar_url_as(format='png', size=512)) as r:
                avys.append(Byteio(await r.read()))
        file = await self.bot.loop.run_in_executor(None, self._quilt, avys)
        await ctx.send(file=file)
    def _quilt(self, avys):
        xbount = math.ceil(math.sqrt(len(avys)))
        ...
        for avy in avys:
            im = Image.open(avy)
            base.paste(im.resize((size, size), resample=PIL.Image.BILINEAR), box=(x * size, y * size))
            if x < xbound - 1:
                x += 1
            else:
                x = 0
